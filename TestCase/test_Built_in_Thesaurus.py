...
@allure.story('内置词库-新增词条-修改词条')
def test_entry_add_85():
    ...
    JsonDic =[{"id": 15309, "entry_bank_name": "com.话费", "standard_value": "测试", "raw_value": "测试1", "status": 1,
              "edit" : "true","key" : 0,"time_create" : time_stamp1,"time_modified" : time_stamp1
              }]
    r = requests.post(url=mt.data_url+"/v1/nlu/entry/add",params = query_params,data=json.dumps(JsonDic))
    print(r.url)
    print(r.json())
...
@allure.story('内置词库-新增词库')
def test_entry_bank_add_86():

    query_params = {
        "access_token" : mt.get_token()
    }

    JsonDic = {"name" : "测试","source" : 1,"tag" : "敏感词", "type" : 1}
    r = requests.post(url=mt.data_url+"/v1/nlu/entry_bank/add",params = query_params,data=json.dumps(JsonDic))
    print(r.json())
    # assert r.json()['code'] == 20000  内置词库不能删除
    with check:
        assert r.status_code == 200
...
@allure.story('内置词库-词库编辑，通用词库转行业词库')
def test_entry_bank_update_87():

    query_params = {
        "access_token" : mt.get_token()
    }
    JsonDic = {"source": 1, "type": 2, "name": "测试", "tag": "敏感词", "industry": 1, "id": 1602}
...
@allure.story('内置词库-导出词库Excel')
def test_export_com_excel_88():

    query_params = {
        "access_token" : mt.get_token()
    }
    r= requests.get(url=mt.data_url+"/v1/nlu/export_com_excel",params=query_params)
    print(r.text)
    with check:
        assert r.status_code == 200
        # assert r.json()['code'] == 20000
...
@allure.story('内置词库-导出语义包')
def test_export_com_nlu_89():
